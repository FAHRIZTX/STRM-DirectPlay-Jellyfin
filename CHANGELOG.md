...
### Changed
- Updated STRM media source metadata to advertise `Container = ts` for HLS direct playback compatibility.
- Kept forced HTTP playback output with `SupportsDirectPlay` and `SupportsDirectStream` enabled.
- Kept placeholder stream codecs as H.264 video and AAC audio for client profile matching.

## [1.0.20] - 2026-06-08

### Changed
- Updated STRM media source metadata to advertise HLS playback with `Container = m3u8`.
- Kept HTTP protocol output with `SupportsDirectPlay` and `SupportsDirectStream` enabled for direct playback paths.
- Updated placeholder media streams to declare H.264 video and AAC audio codecs for client compatibility.

## [1.0.18] - 2026-06-06

### Changed
- Replaced the `Scrutor`-based `IMediaSourceManager` decoration with a manual dependency injection decorator.
- Removed the `Scrutor` NuGet package dependency from the plugin project.

### Fixed
- Fixed plugin startup failing with `Could not load file or assembly 'Scrutor'` when Jellyfin loads plugin services.
- Fixed release packages requiring an external `Scrutor.dll` dependency in the Jellyfin plugin directory.
...
[Unreleased]: https://github.com/fahriztx/STRM-DirectPlay-Jellyfin/compare/v1.0.21...HEAD
[1.0.21]: https://github.com/fahriztx/STRM-DirectPlay-Jellyfin/releases/tag/v1.0.21
